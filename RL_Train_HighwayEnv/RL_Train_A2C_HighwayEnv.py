"""
*******************************************************************************
                            [RL_Train_A2C_HighwayEnv]
*******************************************************************************
File: RL_Train_A2C_HighwayEnv.py
...
A2C  Reinforcement Learning Highway-Env Environment
Training RL A2C Agent on Highway-Env Environment: Tensorboard Log Files
Disclaimer:
------------
...
TIMESTEPS = 2e4
for i in range(1, 3):
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name="A2Clogs",  progress_bar=True)
    # Save the agent
...
timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
video_name = f"A2C-highway_{timestamp}"
